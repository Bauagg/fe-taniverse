...
import {
  ChevronDown, ChevronUp, SlidersHorizontal, AlignLeft,
  MapPin, Tag, CheckCircle, ShieldCheck, Truck, Headphones,
} from "lucide-react";

type SortKey = "terbaru" | "termurah" | "termahal" | "rating";
type StatusFilter = "semua" | "baru" | "hot";

const ALL_CATEGORIES = ["Semua Kategori", ...Array.from(new Set(AGENT_PRODUCTS.map((p) => p.category)))];
const ALL_LOCATIONS = ["Semua Lokasi", ...Array.from(new Set(AGENT_PRODUCTS.map((p) => p.seller.location)))];
...
const FEATURES = [
  { icon: CheckCircle, title: "Produk Berkualitas", desc: "Kualitas terbaik dari petani pilihan" },
  { icon: Truck, title: "Pengiriman Cepat", desc: "Pengiriman aman & tepat waktu" },
  { icon: ShieldCheck, title: "Transaksi Aman", desc: "100% aman dan terpercaya" },
  { icon: Headphones, title: "Dukungan 24/7", desc: "Tim kami siap membantu Anda" },
];
...
export default function AgentPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (category !== "Semua Kategori") list = list.filter((p) => p.category === category);
    if (location !== "Semua Lokasi") list = list.filter((p) => p.seller.location === location);
  ...
  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = { "Semua Kategori": AGENT_PRODUCTS.length };
  ...
  const locationCounts = useMemo(() => {
    const counts: Record<string, number> = { "Semua Lokasi": AGENT_PRODUCTS.length };
    AGENT_PRODUCTS.forEach((p) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="w-full px-4 sm:px-6 py-6">
        ...
        {/* Page Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-extrabold text-gray-900">Marketplace Agent</h1>
          <p className="text-sm text-gray-500 mt-0.5">Temukan berbagai agent terbaik untuk kebutuhan Anda</p>
        </div>
          ...
          <aside className="hidden lg:block w-64 shrink-0">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-20">
                  ...
                  {ALL_CATEGORIES.map((cat) => (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <Tag size={13} className={category === cat ? "text-primary" : "text-gray-400"} />
                        {cat}
                      </span>
              ...
              <SidebarSection title="Status">
                <div className="space-y-1">
                  {([
                    { key: "semua", label: "Semua", count: AGENT_PRODUCTS.length },
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className={`w-2 h-2 rounded-full ${
                          key === "hot" ? "bg-red-500" : key === "baru" ? "bg-primary" : "bg-gray-400"
                        }`} />
                        {label}
                      </span>
                      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
                        status === key ? "bg-primary text-white" : "bg-gray-100 text-gray-500"
                      }`}>
                        {count}
                      </span>
                    </button>
              ...
              <SidebarSection title="Lokasi Penjual">
                <div className="space-y-1">
                  {ALL_LOCATIONS.map((loc) => (
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <MapPin size={13} className={location === loc ? "text-primary" : "text-gray-400"} />
                        {loc}
                      </span>
                      <span className={`text-xs font-semibold px-1.5 py-0.5 rounded-full ${
                        location === loc ? "bg-primary text-white" : "bg-gray-100 text-gray-500"
                      }`}>
              ...
              <SidebarSection title="Rentang Stok" defaultOpen={false}>
                <div className="px-1">
                  <input type="range" min={0} max={50} defaultValue={50} className="w-full accent-primary" />
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center justify-between mb-5">
              <p className="text-sm text-gray-500">
                Menampilkan <span className="font-semibold text-gray-800">1–{filtered.length}</span> dari{" "}
                <span className="font-semibold text-gray-800">{filtered.length}</span> produk
              </p>
                      ...
                      {SORT_OPTIONS.map(({ key, label }) => (
                        <button
                          ...
                          className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                            sort === key ? "text-primary font-semibold bg-primary/5" : "text-gray-700 hover:bg-gray-50"
                          }`}
                        >
                          {label}
                        </button>
            ...
            {filtered.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center py-28 bg-white rounded-2xl border border-gray-100">
                ...
                <p className="text-sm text-gray-400">Coba ubah kategori atau filter lainnya</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-10">
              {FEATURES.map(({ icon: Icon, title, desc }) => (
                <div key={title} className="flex items-start gap-3 bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
