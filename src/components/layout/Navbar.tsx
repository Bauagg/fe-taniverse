...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-100 shadow-sm">
      ...
      {pathname.startsWith("/agent") && (
        <div
          className={`md:hidden overflow-hidden transition-all duration-200 ${searchOpen ? "max-h-16 opacity-100" : "max-h-0 opacity-0"}`}
        >
        ...
        <div className="border-t border-gray-100 bg-white px-4 py-3 flex flex-col gap-1">
          ...
          <div className="flex gap-3 mt-2 pt-3 border-t border-gray-100">
            ...
            <Link
              href="/register"
              onClick={() => setMenuOpen(false)}
              className="flex-1 text-center py-2.5 rounded-full bg-primary text-white text-sm font-semibold hover:bg-green-600 transition-all"
            >
              Daftar
            </Link>
          </div>
